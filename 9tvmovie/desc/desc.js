...
function search(search) {
  ...
  $.getJSON(searchurl + search, function (data) {
    for (var i = 0; i < data.results.length; i++) {if (window.CP.shouldStopExecution(0)) break;
...
function showMovie(choice) {
  ...
  $.getJSON(url + choice + key + "&page=" + next, function (data) {
    for (var i = 0; i < data.results.length; i++) {if (window.CP.shouldStopExecution(1)) break;
      var id = data.results[i].id;
      var title = data.results[i].title;
      var overview = data.results[i].overview;
      var rating = data.results[i].vote_average;
...
function movieInfo(id) {
  $.getJSON(movieCast + id + "/casts?" + key, function (json) {
    ...
    $.getJSON(infoURL, function (data) {
      ...
      var overview = data.overview.slice(0, 120);
      var production_companies = data.production_companies ;
      var production_countries = data.production_countries;
      ...
      $(".item-container").prepend(
        "<div class='overview'><div class='movie-container'><div class='movie-inner'><div class='movie-content'> <div class='col-md-4'><img class='movie-img' src=" +
  backdrop +
  "></div><div class='col-md-6'><li>" +
  overview +
  "</li><br>✧ 𝐅𝐮𝐥𝐥 𝐌𝐨𝐯𝐢𝐞 𝐇𝐃 ✧ 👉 https://youtube.com/redirect?q=YOUR-URL    (If this link doesn't work, check in comment section)<br><br><li>" +
  title + " full movie " +
  year + 
  "</li><li>" +
  tagline +
  ...
  runtime +
  " min</li><li>" +
  title + " " + genres.map(i => i.name).join('<li> '+ data.title + ' full movie ') +
  "</li><li>" +
  title + " " + production_companies.map(i => i.name).join('<li> '+ data.title + ' full movie ') +
  "</li><li>" +
  title + " " + production_countries.map(i => i.name).reverse().join('<li> '+ data.title + ' full movie ') +
  "</li'><li><a class='' onclick='showActor(" +
  cast1id +
  ")'>" +
  cast1ch + " in " + title + " Movie played by " +cast1 + 
  "</a></li><li><a class='' onclick='showActor(" +
  cast2id +
  ")'> " +
  cast2 + " as " + cast2ch +
  "</a></li><li><a class='' onclick='showActor(" +
  ...
  ")'>" +
  cast4 + " as " + cast4ch +
  "</a></li><div id='hideMInfo' class='exit' style='font-size:30px;'><i style='cursor:pointer;' onclick='exit(" +
  id +
...
function showActor(id) {
  ...
  $.getJSON(actorInfo + id + key + "&page=" + next, function (data) {
    for (var i = 0; i < data.results.length; i++) {if (window.CP.shouldStopExecution(2)) break;
      var id = data.results[i].id;
      var title = data.results[i].title;
      var overview = data.results[i].overview;
      var rating = data.results[i].vote_average;
...
function showTv(choice) {
  ...
  $.getJSON(urlTV + choice + key + "&page=" + nextTV, function (data) {
    for (var i = 0; i < data.results.length; i++) {if (window.CP.shouldStopExecution(3)) break;
      var id = data.results[i].id;
...
function tvInfo(id) {
  ...
  $.getJSON(infoURL, function (data) {
    ...
    for (var i = 0; i < data.seasons.length; i++) {if (window.CP.shouldStopExecution(4)) break;
      $(".season").prepend(
...
function seriesInfo(id, num) {
  ...
  $.getJSON(seriesURL, function (data) {
    for (var i = 0; i < data.episodes.length; i++) {if (window.CP.shouldStopExecution(5)) break;
